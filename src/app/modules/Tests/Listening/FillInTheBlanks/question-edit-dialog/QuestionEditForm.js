...
export function QuestionEditForm({
  ...
  useEffect(() => {
    ...
    const inputTextWithoutBlanks = text
      ...
      .split(/\s+/)
      .map((wrd, idx) => {
        if (wrd.replace(/[!"#$%&()*+,-.:;=?@[\]^_`{|}~]/g, "") === "<blank/>") {
          const ans = answer[index];
          // setSelectedWords((prev) => [...prev, { wrd: ans, index: idx }]);
          setSelectedWords((prev) => [
            ...prev,
            { wrd: wrd.replace(/<blank\/>/g, ans), index: idx },
          ]);
          index++;
          return wrd.replace(/<blank\/>/g, ans);
        }
        return wrd;
      })
      .join(" ");
  ...
  const replaceWithBlank = (str) => {
    const position = str
      .split("")
      .map((c, i) => (c.match(/^[A-Za-z0-9_.]+$/i) ? i : null))
      .filter((i) => i !== null);
    return str.replace(str.slice(position[0], position.pop() + 1), "<blank/>");
  };
      ...
      <Formik
        ...
        onSubmit={(values) => {
          ...
          const newValue = {
            ...
            answer: {
              answer: selectedWords?.map((i) =>
                i.wrd.toLocaleLowerCase().replace(/[!"#$%&()*+,-./:;<=>?@[\]^_`{|}~]/g, "")
              ),
            },
          };
...
//       {
//           var c = str.charAt(x);
//           var isLetter = c.match(/[a-z]/i) ? true : false;
//           if(isLetter){
//               startIndex.push(x);
//           }else{
//               endIndex.push(x);
...
//       }
//   var newText = str.replace(str.slice(startIndex[0],endIndex[0]+1),"</blank>")
// }
