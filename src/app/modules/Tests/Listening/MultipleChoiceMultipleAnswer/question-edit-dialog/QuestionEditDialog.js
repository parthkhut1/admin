...
export function QuestionEditDialog({ id, show, onHide }) {
  ...
  const { actionsLoading, questionForEdit } = useSelector(
    (state) => ({
      actionsLoading: state.lMultipleChoiceMultipleAnswers.actionsLoading,
      questionForEdit: state.lMultipleChoiceMultipleAnswers.questionForEdit,
    }),
    shallowEqual
  );
  ...
  const saveQuestion = (question, queryParams) => {
    ...
    formData.append("question_type", "ListeningMultipleChoiceMultipleAnswer");
    ...
    if (!id) {
      ...
      if (question.answer?.answer?.length < 2)
        return enqueueSnackbar("Please select at least 2 question options", {
          variant: "error",
        });
