...
export function QuestionEditDialog({ id, show, onHide }) {
  ...
  const { actionsLoading, questionForEdit } = useSelector(
    (state) => ({
      actionsLoading: state.highlightCorrectSummaries.actionsLoading,
      questionForEdit: state.highlightCorrectSummaries.questionForEdit,
    }),
    shallowEqual
  );
  ...
  const saveQuestion = (question, queryParams) => {
    ...
    formData.append("question_type", "HighlightCorrectSummary");
