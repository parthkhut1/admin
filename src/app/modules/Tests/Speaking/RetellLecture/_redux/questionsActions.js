  ...
  return requestFromServer
    ...
    .then((response) => {
      ...
      if (!questionForCreation.answer.answer) {
        SnackbarUtils.success("Question created Successfully!");
        return dispatch(actions.questionCreated({ question: { question } }));
      }
      ...
      if (questionForCreation.answer.answer)
        formData.append("answer", questionForCreation.answer.answer);
      formData.append(
        "answer_meta[transcript]",
        questionForCreation.answer?.answer_meta?.transcript
      );
      return requestFromServer
        .createAnswer(formData)
        .then((response) => {
...
export const updateQuestion = (question) => (dispatch) => {
  ...
  if (!question.answer.answer && !question.answer?.answer_meta?.transcript) {
    return requestFromServer
      .updateQuestion(question.id, question.question)
      .then((response) => {
        const {
          data: { payload: question1 },
        } = response;
        dispatch(actions.questionUpdated({ question: { question1 } }));
      })
      .catch((error) => {
        dispatch(actions.catchError({ error }));
        throw error;
    ...
    return requestFromServer
      ...
      .then((response) => {
        ...
        console.log("hear3");
        return requestFromServer
          .getAcceptableAnswer(question1.id)
          .then((response) => {
            const {
              data: {
                payload: { data: answer },
            ...
            console.log("hear4/answer", answer);
            ...
            if (question.answer.answer)
              formData.append("answer", question.answer.answer);
            formData.append(
              "answer_meta[transcript]",
            ...
            if (question.answer.answer && answer.length == 0) {
            ...
            return requestFromServer
              .updateAnswer(answer[0].id, formData)
              .then((response) => {
                const {
