      ...
      dispatch(
        actions.questionsFetched({
          ...
          entities: entities.map((e) => ({
            ...
            tags: e.tags.join(),
            text_blanks: e["acceptable-answers"]?.data[0]?.answer.join(","),
          })),
        })
      );
    })
    ...
    return requestFromServer
      ...
      .then((response) => {
        ...
        return requestFromServer
          ...
          .then((response) => {
            return requestFromServer
              .updateAnswer(answer[0].id, question.answer.answer)
              .then((response) => {
                const {
...
export const setBatchInsertModalStatus = (status) => (dispatch) => {
  dispatch(actions.setBatchInsertModalStatus({ status }));
};

export const batchInserQuestion = (file) => (dispatch) => {
  dispatch(actions.startCall({ callType: callTypes.action }));
  return requestFromServer
    .batchInserQuestion(file)
    .then(() => {
      SnackbarUtils.success("Insertion was successfully.");
    ...
    .catch((error) => {
      dispatch(actions.catchError({ error }));
        ...
        SnackbarUtils.error(error.response.data.message);
      SnackbarUtils.error(error.message?`${error.message},uploaded file must be in selected question type.`:"Please try agin, uploaded file must be in selected question type.");
      throw error;
    });
};
