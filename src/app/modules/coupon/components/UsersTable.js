...
export default function EnhancedTable({
  ...
  const { total, currentPageUsers } = useSelector(
    (state) => ({
      currentPageUsers: state.coupons.currentPageUsers,
      total: state.coupons.totalCountOfFilteredUsers,
    }),
    shallowEqual
  );
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = filterdUsers.map((n) => ({
        id: n.id,
        name: n.name,
        email: n.email,
      }));
  ...
  const emptyRows =
    rowsPerPage -
    Math.min(rowsPerPage, filterdUsers.length - page * rowsPerPage);
