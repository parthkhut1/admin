...
export default function EnhancedTable({
  ...
  const { total, currentPagePackages } = useSelector(
    (state) => ({
      currentPagePackages: state.coupons.currentPagePackages,
      total: state.coupons.totalCountOfFilteredPackages,
    }),
    shallowEqual
  );
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = filteredPackages.map((n) => ({
        id: n.id,
        name: n.name,
  ...
  const emptyRows =
    rowsPerPage -
    Math.min(rowsPerPage, filteredPackages.length - page * rowsPerPage);
      ...
      {filteredPackages.length !== 0 ? (
        <>
          ...
          <TablePagination
            ...
            page={currentPagePackages - 1}
            onChangePage={(e, number) => {
              handleChangePage(e, number);
              dispatch(
                actions.fetchFilteredPackages({
                  ...filterValues,
                  page: number + 1,
