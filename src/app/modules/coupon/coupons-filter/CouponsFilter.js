...
const prepareFilter = (queryParams, values) => {
  ...
  if (searchText) {
    ...
    filter.token = searchText;
  }
  newQueryParams.filter = filter;
  return newQueryParams;
};

export function CouponsFilter({ listLoading }) {
  // Coupons UI Context
  ...
  const applyFilter = (values) => {
    const newQueryParams = prepareFilter(couponsUIProps.queryParams, values);
    if (!isEqual(newQueryParams, couponsUIProps.queryParams)) {
      newQueryParams.pageNumber = 1;
      // update list by queryParams
      couponsUIProps.setQueryParams(newQueryParams);
    }
  };
