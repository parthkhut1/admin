...
export default function EnhancedTable({
  ...
  const { total, currentPageSessions } = useSelector(
    (state) => ({
      currentPageSessions: state.packages.currentPageSessions,
      total: state.packages.totalCountOfFilteredSessions,
    }),
    shallowEqual
  );
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = filterdSessions.map((n) => ({
        id: n.id,
        name: n.name,
  ...
  const emptyRows =
    rowsPerPage -
    Math.min(rowsPerPage, filterdSessions.length - page * rowsPerPage);

  return (
    <div className={classes.root}>
      {filterdSessions.length !== 0 ? (
        <>
          ...
          <TableContainer>
            ...
            >
              <EnhancedTableHead
                ...
                rowCount={filterdSessions.length}
              />
              <TableBody>
                {filterdSessions
          ...
          <TablePagination
            ...
            page={currentPageSessions - 1}
            onChangePage={(e, number) => {
              handleChangePage(e, number);
              dispatch(
                actions.fetchFilteredSessions({
                  ...filterValues,
                  page: number + 1,
