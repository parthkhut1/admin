...
export default function EnhancedTable({
  ...
  const { total, currentPageQuestions } = useSelector(
    (state) => ({
      currentPageQuestions: state.packages.currentPageQuestions,
      total: state.packages.totalCountOfFilteredQuestions,
    }),
    shallowEqual
  );
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = filterdQuestions.map((n) => ({
        id: n.id,
        title: n.title,
